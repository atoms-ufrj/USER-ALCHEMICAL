...
PairMIECutLondon::PairMIECutLondon(LAMMPS *lmp) : Pair(lmp)
{
  respa_enable = 1;
}
...
PairMIECutLondon::~PairMIECutLondon()
{
  if (allocated) {
...
void PairMIECutLondon::compute(int eflag, int vflag)
{
...
void PairMIECutLondon::compute_inner()
{
  int i,j,ii,jj,inum,jnum,itype,jtype;
...
void PairMIECutLondon::compute_middle()
{
  int i,j,ii,jj,inum,jnum,itype,jtype;
  double xtmp,ytmp,ztmp,delx,dely,delz,fpair;
  double rsq,r2inv,rgamR,rgamA,forcemie,factor_mie,rsw;
...
void PairMIECutLondon::compute_outer(int eflag, int vflag)
{
  int i,j,ii,jj,inum,jnum,itype,jtype;
  double xtmp,ytmp,ztmp,delx,dely,delz,evdwl,fpair;
  double rsq,r2inv,rgamR,rgamA,forcemie,factor_mie,rsw;
...
void PairMIECutLondon::allocate()
{
  allocated = 1;
  int n = atom->ntypes;
...
void PairMIECutLondon::settings(int narg, char **arg)
{
...
void PairMIECutLondon::coeff(int narg, char **arg)
{
  if (narg < 5 || narg > 6)
...
void PairMIECutLondon::init_style()
{
...
void PairMIECutLondon::init_list(int id, NeighList *ptr)
{
  if (id == 0) list = ptr;
...
double PairMIECutLondon::init_one(int i, int j)
{
  if (setflag[i][j] == 0) {
...
void PairMIECutLondon::write_restart(FILE *fp)
{
...
void PairMIECutLondon::read_restart(FILE *fp)
{
...
void PairMIECutLondon::write_restart_settings(FILE *fp)
{
...
void PairMIECutLondon::read_restart_settings(FILE *fp)
{
  int me = comm->me;
  if (me == 0) {
...
double PairMIECutLondon::single(int i, int j, int itype, int jtype, double rsq,
...
void *PairMIECutLondon::extract(const char *str, int &dim)
{
  dim = 2;
