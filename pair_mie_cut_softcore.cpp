...
PairMieCutSoftcore::PairMieCutSoftcore(LAMMPS *lmp) : PairAlchemical(lmp)
{
  respa_enable = 1;
  writedata = 1;
}
...
PairMieCutSoftcore::~PairMieCutSoftcore()
{
  if (allocated) {
...
void PairMieCutSoftcore::compute(int eflag, int vflag)
{
    ...
    for (jj = 0; jj < jnum; jj++) {
      ...
      if (rsq < cutsq[itype][jtype]) {
        ...
        forcemie = mie2[itype][jtype] * sinvc * ratio *
            (gamR[itype][jtype]*sinvcRA - gamA[itype][jtype]*sinvc);
...
void PairMieCutSoftcore::compute_inner()
{
...
void PairMieCutSoftcore::compute_middle()
{
  int i,j,ii,jj,inum,jnum,itype,jtype;
  double xtmp,ytmp,ztmp,delx,dely,delz,fpair;
  double rsq,ratio,sinvc,rgamA,forcemie,factor_mie,rsw;
...
void PairMieCutSoftcore::compute_outer(int eflag, int vflag)
{
  int i,j,ii,jj,inum,jnum,itype,jtype;
  double xtmp,ytmp,ztmp,delx,dely,delz,evdwl,fpair;
  double rsq,ratio,sinvc,rgamA,forcemie,factor_mie,rsw;
...
void PairMieCutSoftcore::allocate()
{
  allocated = 1;
  int n = atom->ntypes;
...
void PairMieCutSoftcore::settings(int narg, char **arg)
{
...
void PairMieCutSoftcore::coeff(int narg, char **arg)
{
  if (narg < 6 || narg > 7)
...
void PairMieCutSoftcore::init_style()
{
...
void PairMieCutSoftcore::init_list(int id, NeighList *ptr)
{
  if (id == 0) list = ptr;
...
double PairMieCutSoftcore::atanx_x(double x)
{
  double y,z,d,s,t;
  y = -x*x;
  z = d = s = 1.0;
...
double PairMieCutSoftcore::init_one(int i, int j)
{
  ...
  gamA[j][i] = gamA[i][j];
  gamR[j][i] = gamR[i][j];
  rcA = pow(cut[i][j], gamA[i][j]);
  Cmie = (gamR[i][j]/(gamR[i][j]-gamA[i][j]) *
                pow((gamR[i][j]/gamA[i][j]),
                    (gamA[i][j]/(gamR[i][j]-gamA[i][j]))));

  mie1[i][j] = mie1[j][i] = pow(sigma[i][j], gamA[i][j]);
  mie2[i][j] = mie2[j][i] = Cmie*epsilon[i][j] *pow(lambda,exponent_n);
  mie3[i][j] = mie3[j][i] = gamR[i][j]/gamA[i][j];
...
void PairMieCutSoftcore::write_restart(FILE *fp)
{
...
void PairMieCutSoftcore::read_restart(FILE *fp)
{
...
void PairMieCutSoftcore::write_restart_settings(FILE *fp)
{
...
void PairMieCutSoftcore::read_restart_settings(FILE *fp)
{
  int me = comm->me;
  if (me == 0) {
...
void PairMieCutSoftcore::write_data(FILE *fp)
{
...
void PairMieCutSoftcore::write_data_all(FILE *fp)
{
  for (int i = 1; i <= atom->ntypes; i++)
...
double PairMieCutSoftcore::single(int i, int j, int itype, int jtype, double rsq,
...
void *PairMieCutSoftcore::extract(const char *str, int &dim)
{
  dim = 2;
