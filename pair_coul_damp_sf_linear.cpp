...
#include "string.h"
#include "pair_coul_damp_sf_linear.h"
#include "atom.h"
#include "comm.h"
#include "force.h"
#include "neighbor.h"
...
PairCoulDampSFLinear::PairCoulDampSFLinear(LAMMPS *lmp) : PairAlchemical(lmp)
{
  single_enable = 1;
  self_flag = 0;
}
...
void PairCoulDampSFLinear::compute(int eflag, int vflag)
{
...
void PairCoulDampSFLinear::allocate()
{
  allocated = 1;
  int n = atom->ntypes;
...
void PairCoulDampSFLinear::settings(int narg, char **arg)
{
...
void PairCoulDampSFLinear::reinit()
{
...
double PairCoulDampSFLinear::init_one(int i, int j)
{
  return cut_coul;
}
...
void PairCoulDampSFLinear::modify_params(int narg, char **arg)
{
  if (narg == 0)
...
void PairCoulDampSFLinear::write_restart(FILE *fp)
{
...
void PairCoulDampSFLinear::read_restart(FILE *fp)
{
...
void PairCoulDampSFLinear::write_restart_settings(FILE *fp)
{
...
void PairCoulDampSFLinear::read_restart_settings(FILE *fp)
{
  if (comm->me == 0) {
...
double PairCoulDampSFLinear::single(int i, int j, int itype, int jtype, double rsq,
...
void *PairCoulDampSFLinear::extract(const char *str, int &dim)
{
